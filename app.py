...
# --- CONEXÃO INDIVIDUAL (VERSÃO SIMPLIFICADA) ---
@st.cache_resource
def get_gspread_client():
    try:
        # Puxa o JSON bruto do segredo único JSON_CHAVE
        if "JSON_CHAVE" in st.secrets:
            info = json.loads(st.secrets["JSON_CHAVE"])
        else:
            st.error("Erro: Segredo 'JSON_CHAVE' não encontrado no Streamlit Cloud.")
            return None
        ...
        st.error(f"Falha na Autenticação (JWT): {e}")
        return None
...
def calculate_projections(sessions_df, overrides_df):
    ...
    for (materia, assunto), group in groups:
        latest = group.iloc[-1]; initial = group.iloc[0]; num = len(group)
...
with tab2:
    ...
    with st.form("form_reg", clear_on_submit=True):
        ...
        if st.form_submit_button("Salvar Registro"):
            new_r = {'data': datetime.now().strftime('%Y-%m-%d'), 'materia': m_in, 'assunto': a_in, 'total': t_in, 'acertos': ac_in, 'timestamp': datetime.now().timestamp(), 'erros': str(e_in)}
